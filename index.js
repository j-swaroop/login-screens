...
passwordField.addEventListener("input", (e) => {
  ...
  if (isLengthValid && hasUppercase && hasNumber && hasNumber){
    passwordField.classList.remove('error-state')
  }else{
...
dropdownContainer.addEventListener('blur', ({relatedTarget}) => {
  if (dropdownContainer && (dropdownContainer.contains(relatedTarget) || dropdownContainer.isEqualNode(relatedTarget))) {
    return
  }
  dropdownMenu.style.display = "none";
})
...
signUpForm.addEventListener('submit', (event) => {
  ...
  const isAllEmpty = !userData.name.trim() && !userData.email.trim() && !userData.password.trim() && !userData.phone.trim();
